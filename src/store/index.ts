...
export class IdiomStore {
  ...
  private startQuiz(forcedSize?: number, isPracticeMode: boolean = false): void {
    ...
    const learned = seenWithMeta.filter((item: { idiom: Idiom | null; meta: IdiomMeta }) => item.meta.dateLearned);

    // Sort by priority:
    // 1. Prefer un-quizzed idioms (those with no quizHistory or empty quizHistory)
    // 2. Secondary: prefer older-learned idioms
    const sortByPriority = (items: Array<{ idiom: Idiom | null; meta: IdiomMeta }>) => {
      return items.sort((a: { idiom: Idiom | null; meta: IdiomMeta }, b: { idiom: Idiom | null; meta: IdiomMeta }) => {
    ...
    const selectedIds = prioritizedPool
      .slice(0, quizSize)
      .map((item: { idiom: Idiom | null; meta: IdiomMeta }) => item.idiom!.id);
  ...
  private generateClozeQuestion(idiom: Idiom): QuizQuestion {
  ...
  private generateUsageIdentification(idiom: Idiom): UsageIdentificationQuestion {
    ...
    const sentences: string[] = [correctSentence];
  ...
  private generateTrueFalse(idiom: Idiom): TrueFalseQuestion {
  ...
  private generateWordOrder(idiom: Idiom): WordOrderQuestion {
  ...
  private validateAnswer(answer: string, question: QuizQuestion): boolean {
    ...
    if (question.type === 'word-order') {
      ...
      const correctWords = (question as any).correctOrder || question.correctAnswer.split(' ');
  ...
  getQuestionFeedback(questionIndex: number): QuestionFeedback | null {
    if (!this.state.quizInProgress?.feedback) return null;
  ...
  completeQuiz(): QuizResult & { newIdiom: Idiom | null } {
    ...
    quiz.idiomIds.forEach((id) => {
      ...
      if (meta) {
        if (!meta.quizHistory) meta.quizHistory = [];
