...
export class ChunkManager {
  private loadedChunks: Map<number, Idiom[]> = new Map();
  private loadingPromises: Map<number, Promise<Idiom[]>> = new Map();
  private preloadedChunks: Set<number> = new Set();

  /**
   * Load a chunk file and cache it in memory
   */
  async loadChunk(chunkIndex: number): Promise<Idiom[]> {
    ...
    const loadPromise = (async () => {
      try {
        ...
        const idioms: Idiom[] = await response.json();

        // Validate loaded data
        if (!Array.isArray(idioms) || idioms.length !== chunkMeta.count) {
          console.warn(
            `Chunk ${chunkIndex} size mismatch: expected ${chunkMeta.count}, got ${idioms.length}`
          );
        }
   ...
   */
  async getIdiomAtGlobalIndex(globalIndex: number): Promise<Idiom | null> {
   ...
   */
  async getIdiomsByIds(idiomIds: string[]): Promise<Idiom[]> {
    // Build a map of chunks we need to load
    const neededChunks = new Set<number>();
    const idiomMap = new Map<string, Idiom>();
    ...
    for (const idiomId of idiomIds) {
      if (idiomMap.has(idiomId)) continue;
    ...
    await Promise.all(
      Array.from(neededChunks).map(chunkIndex => this.loadChunk(chunkIndex))
    );
   ...
   */
  getCacheStats(): { loadedChunks: number; totalChunks: number; loadedIdioms: number } {
    let loadedIdioms = 0;
