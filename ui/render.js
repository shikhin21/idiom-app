...
function renderTodayIdioms(idioms, learnedIds) {
    if (idioms.length === 0) {
...
function renderIdiomCard(idiom, isFirst, isLearned) {
...
function renderQuizQuestion(questions, index) {
    ...
    clearElement(footer);
    footer.appendChild(div({ className: 'quiz-footer__buttons' }, [
        isSubmitted
...
function renderInlineFeedbackBanner(feedback, question) {
    const isCorrect = feedback.isCorrect ?? false;
    return div({ className: `feedback-banner ${isCorrect ? 'feedback-banner--correct' : 'feedback-banner--incorrect'}` }, [
...
function renderStandardMCQ(question, index, progress) {
    ...
    const elements = [
        ...
        div({ className: 'quiz-question__options' }, question.options.map((option, i) => renderQuizOption(option, i, currentQuizAnswers[index] === option, question.correctAnswer, isSubmitted))),
    ];
...
function renderUsageIdentification(question, index, progress) {
    ...
    const elements = [
        ...
        div({ className: 'quiz-question__options' }, question.sentences.map((sentence, i) => renderQuizOption(sentence, i, currentQuizAnswers[index] === sentence, question.correctAnswer, isSubmitted))),
    ];
    // Add feedback banner if submitted
    if (isSubmitted && feedback) {
        elements.push(renderInlineFeedbackBanner(feedback, question));
...
function renderWordOrder(question, index, progress) {
    ...
    const selectedWords = selectedIndices.map((i) => question.shuffledWords[i]);
    ...
    const elements = [
        ...
        div({ className: 'word-order__selected' }, [
            ...
            div({ className: 'word-order__chips' }, selectedWords.length > 0
    ...
    if (!isSubmitted) {
        elements.push(div({ className: 'word-order__available' }, [
...
function renderQuizOption(text, index, isSelected, correctAnswer, isSubmitted) {
...
function renderQuizResult(result, questions, feedback) {
    ...
    clearElement(footer);
    footer.appendChild(result.passed
        ? button({
            className: 'btn btn--primary',
...
function renderNewIdiomPreview(idiom) {
